...
var BaasBox = (function() {
    ...
    var COOKIE_KEY_ACTION = "cookie_key_action";
    ...
    function setCurrentAction(actionObject) {
        ...
        // if the user is using Zepto, then local storage must be used (if supported by the current browser)
        if (window.Zepto && window.localStorage) {
          window.localStorage.setItem(COOKIE_KEY_ACTION, JSON.stringify(this.action));
        } else {
          $.cookie(COOKIE_KEY_ACTION, JSON.stringify(this.action));
        }
      }
    ...
    function getCurrentAction() {
        // if the user is using Zepto, then local storage must be used (if supported by the current browser)
        if (window.Zepto && window.localStorage) {
          if (localStorage.getItem(COOKIE_KEY_ACTION)) {
            this.action = JSON.parse(localStorage.getItem(COOKIE_KEY_ACTION));
          }
        } else {
          if ($.cookie(COOKIE_KEY_ACTION)) {
            this.action = JSON.parse($.cookie(COOKIE_KEY_ACTION));
          }
        }
        return this.action;
      }
    ...
    return {
      ...
      wechatLogin: function(code, state) {
          var deferred = buildDeferred();
          var url = BaasBox.endPoint + '/wechatLogin\?code='+code+"&state="+state;
          var loginRequest = $.get(url)
          .done(function(res) {
            var roles = [];
            $(res.data.user.roles).each(function(idx, r) {
                roles.push(r.name);
...
//      },
      ...
      loadCurrentAction: function() {
          var deferred = buildDeferred();
          var url = BaasBox.endPoint + '/document/Action';
          var req = $.ajax({
              url: url,
              method: 'GET',
              timeout: BaasBox.timeout,
              dataType: 'json',
              data: {
  				where : "isCurrentAction=true"
  			}
          })
            .done(function(res) {
//              deferred.resolve(res['data']);
            	setCurrentAction(res['data'][0]);
                deferred.resolve(getCurrentAction());
            })
            .fail(function(error) {
              deferred.reject(error);
            })
