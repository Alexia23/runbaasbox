...
$("#displayAllUsers").bind("click", function () {
	callMenu('#verifications');
});

function setUserVerifyStatus(userInfo,verifyStatus,noPassReason){
	userInfo.visibleByTheUser.verifyStatus = verifyStatus;
	if (verifyStatus == "3")
		userInfo.visibleByTheUser.noPassReason = noPassReason;
	var userName = userInfo.user.name;
	var visibleByTheUser = reverseJSON(userInfo.visibleByTheUser);
	var visibleByFriends = reverseJSON(userInfo.visibleByFriends);
	var visibleByRegisteredUsers = reverseJSON(userInfo.visibleByRegisteredUsers);
	var visibleByAnonymousUsers = reverseJSON(userInfo.visibleByAnonymousUsers);
...
function setCurrentAction(actionId){
	BBRoutes.com.baasbox.controllers.Admin.setCurrentAction(actionId).ajax(
			{
				error: function(data)
				{
					alert(JSON.parse(data.responseText)["message"]);
...
$(".btn-action").live("click", function() {
	...
	switch (action)	{
	case "pass":
        if(!confirm("你确定让用户 '"+actionType+"' 通过审核?")) return;
		setUserVerifyStatus(JSON.parse(parameters), "2");
        break;
	case "no pass":
		var noPassReason = prompt("请输入用户 '"+actionType+"' 审核不通过的原因：", "");
        if(noPassReason == null) return;
        if(noPassReason == "") {
        	alert('审核不通过的原因不能为空！');
            return;
        }        	
		setUserVerifyStatus(JSON.parse(parameters), "3", noPassReason);
        break;
	case "setCurrentAction":
		setCurrentAction(parameters);
        break;
	case "enable":
...
function populateActionEditForm(docObject){
	$("#txtActionId").val(docObject.id);
	$("#txtActionName").val(docObject.name);
	$("#txtActionStartTime").val(docObject.startTime);
	$("#txtActionEndTime").val(docObject.endTime);
	$("#txtActionTotalDistances").val(docObject.totalDistances);	
	$("#txtActionDescription").val(docObject.description);
}

function populateTeamEditForm(docObject){
	$("#txtTeamId").val(docObject.id);
	$("#txtTeamName").val(docObject.name);
	$("#txtTeamDescription").val(docObject.description);
}
...
function openActionEditForm(id){
	var docObject;
	resetAddActionForm();
	$("#actionTitle").text(id!=null?"Edit Action":"Create New Action");
	$('#actionModalMode').text(id!=null?"edit":"insert");
	docObject = {}
	if(id!=null){
		for(i=0;i<actionDataArray.length;i++)
		{
			if(actionDataArray[i].id == id)
				docObject = actionDataArray[i];
		}
		populateActionEditForm(docObject);
	}else{
		docObject["@class"] = "Action";
	}	
...
function openTeamEditForm(id){
	var docObject;
	resetAddTeamForm();
	$("#teamTitle").text(id!=null?"Edit Team":"Create New Team");
	$('#teamModalMode').text(id!=null?"edit":"insert");
	docObject = {}
	if(id!=null){
		for(i=0;i<teamDataArray.length;i++)
		{
			if(teamDataArray[i].id == id)
				docObject = teamDataArray[i];
		}
		populateTeamEditForm(docObject);
	}else{
		docObject["@class"] = "Team";
	}	
...
function resetAddActionForm(){
	// console.debug("resetAddActionForm");
	$("#actionForm")[0].reset();
	$("#txtActionStartTime").val(new Date().format('yyyy-MM-dd'));
	$("#txtActionEndTime").val(new Date(new Date().valueOf() + 100 * 24 * 60 * 60 * 1000).format('yyyy-MM-dd'));
	$("#txtActionStartTime").datepicker({
		dateFormat: 'yy-mm-dd'
	});
	$("#txtActionEndTime").datepicker({
		dateFormat: 'yy-mm-dd'
	});
	$("#errorAddAction").addClass("hide");
	$("#errorAddAction2").addClass("hide");
	$(".error").removeClass("error");
}

function resetAddTeamForm(){
	// console.debug("resetAddTeamForm");
	$("#teamForm")[0].reset();
	$("#errorAddTeam").addClass("hide");
	$("#errorAddTeam2").addClass("hide");
	$(".error").removeClass("error");
}
...
function addAction() {
	...
	var actionStartTime = $("#txtActionStartTime").val();
	var actionEndTime = $("#txtActionEndTime").val();
	var actionTotalDistances = $("#txtActionTotalDistances").val();
	var actionDescription = $("#txtActionDescription").val();

	BBRoutes.com.baasbox.controllers.Document.createDocument("Action").ajax(
			{
				data: JSON.stringify({"name": actionName,
					"startTime": actionStartTime,
					"endTime": actionEndTime,
					"totalDistances": actionTotalDistances,
				...
				error: function(data)
				{
					...
					if (bb_code=="40001") message += " HINT: reload the action from the server and redo your update";
					$("#errorAddAction").text(message).removeClass("hide");
				},
				success: function(data)
				{
					closeActionForm();
					BBRoutes.com.baasbox.controllers.Document.grantToRole("Action", data.data["id"], "registered", "read", true).ajax(
							{
								error: function(data)
								{
									var error=JSON.parse(data.responseText);
...
function updateAction(){
	...
	BBRoutes.com.baasbox.controllers.Document.updateDocument("Action",id,true).ajax(
			{
				data: JSON.stringify({"name": actionName,
					"startTime": actionStartTime,
					"endTime": actionEndTime,
...
function addTeam() {
	...
	var teamDescription = $("#txtTeamDescription").val();

	BBRoutes.com.baasbox.controllers.Document.createDocument("Team").ajax(
			{
				...
				error: function(data)
				{
					...
					var message=error["message"];
					var bb_code=error["bb_code"];
					if (bb_code=="40001") message += " HINT: reload the team from the server and redo your update";
					$("#errorAddTeam").text(message).removeClass("hide");
				},
				success: function(data)
				{
					closeTeamForm();
					BBRoutes.com.baasbox.controllers.Document.grantToRole("Team", data.data["id"], "registered", "read", true).ajax(
							{
								error: function(data)
								{
									var error=JSON.parse(data.responseText);
...
function updateTeam(){
	var id=$("#txtTeamId").val();
	var teamName = $("#txtTeamName").val();
	var teamDescription = $("#txtTeamDescription").val();

	BBRoutes.com.baasbox.controllers.Document.updateDocument("Team",id,true).ajax(
			{
				data: JSON.stringify({"name": teamName,
					"description": teamDescription}),
				contentType: "application/json",
...
$('.btn-ActionCommit').click(function(e){
	var errorMessage = '';
	$("#errorAddAction").addClass("hide");
	var action=$('#actionModalMode').text();
	var actionName = $("#txtActionName").val();
	var actionStartTime = $("#txtActionStartTime").val();
	var actionEndTime = $("#txtActionEndTime").val();
	var actionTotalDistances = $("#txtActionTotalDistances").val();
	...
	if($.trim(actionName) == "") {
		errorMessage = "The 'Action name' field is required<br/>"
	} else if($.trim(actionStartTime) == "") {
		errorMessage = "The 'Action startTime' field is required<br/>"
	} else if($.trim(actionEndTime) == "") {
		errorMessage = "The 'Action endTime' field is required<br/>"
	} else if($.trim(actionTotalDistances) == "") {
		errorMessage = "The 'Action totalDistances' field is required<br/>"
	}
			
	if(errorMessage != "")
	{
		$("#errorAddAction").html(errorMessage);
		$("#errorAddAction").removeClass("hide");
		return;
	}

	if(action == "insert")	addAction();
	else updateAction();
	return;
}); // Validate and Ajax submit for Insert/Update Action
$('.btn-ActionReload').click(function(e){
	var errorMessage = '';
	$("#errorAddAction").addClass("hide");

	var id=$("#txtActionId").val();

	BBRoutes.com.baasbox.controllers.Document.getDocument("Action",id,true).ajax(
			{
				error: function(data)
				{
					var error=JSON.parse(data.responseText);
					...
					$("#errorAddAction").text(message);
					$("#errorAddAction").removeClass("hide");
				},
				success: function(data)	{
				    populateActionEditForm(data["data"]);
				    callMenu("#actions");
				}
			});
	return;
}); // Validate and Ajax submit for reload action

$('.btn-TeamCommit').click(function(e){
	var errorMessage = '';
	$("#errorAddTeam").addClass("hide");
	var action=$('#teamModalMode').text();
	var teamName = $("#txtTeamName").val();
	var teamDescription = $("#txtTeamDescription").val();
	
	if($.trim(teamName) == "") {
		errorMessage = "The 'Team name' field is required<br/>"
	}
			
	if(errorMessage != "")
	{
		$("#errorAddTeam").html(errorMessage);
		$("#errorAddTeam").removeClass("hide");
		return;
	}

	if(action == "insert")	addTeam();
	else updateTeam();
	return;
}); // Validate and Ajax submit for Insert/Update Team
$('.btn-TeamReload').click(function(e){
	var errorMessage = '';
	$("#errorAddTeam").addClass("hide");

	var id=$("#txtTeamId").val();

	BBRoutes.com.baasbox.controllers.Document.getDocument("Team",id,true).ajax(
			{
				error: function(data)
				{
					var error=JSON.parse(data.responseText);
					var message=error["message"];
					alert(JSON.parse(message));
					$("#errorAddTeam").text(message);
					$("#errorAddTeam").removeClass("hide");
				},
				success: function(data)	{
				    populateTeamEditForm(data["data"]);
				    callMenu("#teams");
				}
			});
	return;
}); // Validate and Ajax submit for reload team
...
function setupTables(){
	$('#verificationTable').dataTable( {
		...
		"aoColumns": [{"mData": "visibleByTheUser.name"},
		              {"mData": "visibleByTheUser.headImgUrl", "mRender": function ( data, type, full ) {
		            	   	if (data) return "<img style='width:50px' src='" + data + "'/>";
		            	   	return "";
						}
					   },
					   {"mData": "visibleByTheUser.nickName", "mRender": function ( data, type, full ) {
		            	   if(data) return emoji.replace_unified(data);
		            	   return "";
						}
					   },
					   {"mData": "visibleByTheUser.classAndGrade"},
		               {"mData": "visibleByTheUser.company"},
		               {"mData": "visibleByTheUser.phone"},
		               {"mData": "visibleByTheUser.teamName"},
		               {"mData": "visibleByTheUser.verifyStatus", "mRender": function ( data, type, full ) {
		            	   	switch (data){
			            		case "1":
			            			return "等待审核";
			            			break;
		               ...
		               {"mData": "visibleByTheUser.verifyStatus", "mRender": function ( data, type, full ) {
		            	   if(data == "1")
		            		   return getActionButton("pass",full.visibleByTheUser.name,JSON.stringify(full)) +" "+ getActionButton("no pass",full.visibleByTheUser.name,JSON.stringify(full));
		            	   return "";
		               }
		               }],
       "bRetrieve": true,
...
//        "oLanguage": {"sLengthMenu": "_MENU_ records per page"},
	...
	$('#btnReloadActions').click(function(){
		callMenu('#actions');
	});
	$('#btnReloadTeams').click(function(){
		callMenu('#teams');
	});
	$('#btnReloadVerifications').click(function(){
		callMenu('#verifications');
	});
	$('#btnReloadRunDatas').click(function(){
		callMenu('#rundatas');
	});
...
function setupSelects(){
	...
	$("#selectActionsName").chosen().change(function(){
		if ($('#selectActionsName').has('option').length>0){
			val=$("#selectActionsName").val();
			//console.log('collName length ' + $('#selectActionsName').has('option').length);
//			loadDocumentsData(val);
			loadRunDatasData();
//			var scope=$("#rundatas").scope();
//			scope.$apply(function(){
//				scope.collectionName=val;
//			});
		}//dropdown option has not been selected yet
	...
	$("#selectTeamsName").chosen().change(function(){
		if ($('#selectTeamsName').has('option').length>0){
			val=$("#selectTeamsName").val();
			//console.log('collName length ' + $('#selectTeamsName').has('option').length);
			loadRunDatasData();
//			var scope=$("#rundatas").scope();
//			scope.$apply(function(){
//				scope.collectionName=val;
...
function callMenu(action){
	...
	switch (action)	{
	case "#actions":		
		resetDataTable($('#actionTable'));
		loadActionsData();   
		applySuccessMenu(action,actionDataArray);
		break; // #actions
	case "#teams":		
		resetDataTable($('#teamTable'));
		loadTeamsData();   
		applySuccessMenu(action,teamDataArray);
		break; // #teams
	case "#verifications":
		var query = {where: "visibleByTheUser.name <> ''"};
		if (!$("#displayAllUsers")[0].checked) {
			query = {where: "visibleByTheUser.verifyStatus = '1' and visibleByTheUser.name <> ''"};
		}
		BBRoutes.com.baasbox.controllers.Admin.getUsers().ajax({
			data: query,
			success: function(data) {
				verificationDataArray = data["data"];
				//console.debug("Admin.getUsers() success:");
				//console.debug(data);
				applySuccessMenu(action,verificationDataArray);
				$('#verificationTable').dataTable().fnClearTable();
				$('#verificationTable').dataTable().fnAddData(verificationDataArray);
			}
		});
		break; // #verifications
	case "#rundatas":
		resetDataTable( $('#rundataTable'));
		BBRoutes.com.baasbox.controllers.Document.getDocuments("Action").ajax({
			data: {orderBy: "endTime desc"},
			success: function(data) {
				data=data["data"];
				applySuccessMenu(action,data);
				var sel=$('#selectActionsName');
				sel.empty();

				resetChosen("#selectActionsName");
...
//				sel.append($("<option/>"));
				...
				BBRoutes.com.baasbox.controllers.Document.getDocuments("Team").ajax({
					success: function(data) {
						data=data["data"];
						applySuccessMenu(action,data);
						var selTeam=$('#selectTeamsName');
						selTeam.empty();

						resetChosen("#selectTeamsName");
...
function VerificationsController($scope){}
function RunDatasController($scope){}
